// CS4803 (Spring 2010) Lab 3 — 2D convolution, SHARED + CONSTANT version.
// Ported from CS4803VGCD/lab3/lab3_shared_constant/2Dconvolution_kernel.cu.
// Same 20x20/16x16 tiling as the shared version, but the 5x5 filter lives in
// __constant__ memory (uploaded once per call via cudaMemcpyToSymbol).
...
__constant__ float Ms[KERNEL_SIZE * KERNEL_SIZE];
    ...
    if (2 <= ty && 2 <= tx && ty <= 17 && tx <= 17) {
        for (unsigned int m = 0; m < 5; ++m)
            for (unsigned int n = 0; n < 5; n++)
                result += Ms[m * 5 + n] * Ns[ty + m - 2][tx + n - 2];
...
extern "C" void solution(const float* filter, const float* N, float* P, size_t size) {
    cudaMemcpyToSymbol(Ms, filter, KERNEL_SIZE * KERNEL_SIZE * sizeof(float));
