// Metal shader for "mean-subtract": one threadgroup per row, tree reduction over the row.
...
kernel void solution(device const float* X [[buffer(0)]],
    ...
    float acc = 0.0f;
    for (uint i = tid; i < D; i += tpg) { float v = rx[i]; acc = acc + v; }
    smem[tid] = acc;
