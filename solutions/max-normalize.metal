// Metal shader for "max-normalize": one threadgroup per row, tree reduction over the row.
...
kernel void solution(device const float* X [[buffer(0)]],
    ...
    float acc = 0.0f;
    for (uint i = tid; i < D; i += tpg) { float v = rx[i]; acc = max(acc, fabs(v)); }
    smem[tid] = acc;
    threadgroup_barrier(mem_flags::mem_threadgroup);
    for (uint s = tpg / 2; s > 0; s >>= 1) {
        if (tid < s) { float a = smem[tid], b = smem[tid + s]; smem[tid] = max(a, b); }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
    float f;
    { float s = smem[0]; f = s; }
