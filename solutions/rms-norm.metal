// Metal shader for "rms-norm": one threadgroup per row, tree reduction over the row.
...
kernel void solution(device const float* X [[buffer(0)]],
    ...
    float acc = 0.0f;
    for (uint i = tid; i < D; i += tpg) { float v = rx[i]; acc = acc + v * v; }
    smem[tid] = acc;
    ...
    float f;
    { float s = smem[0]; f = rsqrt(s / float(D) + 1e-5f); }
