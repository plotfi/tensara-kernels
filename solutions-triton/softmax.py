"""Triton solution for `softmax` — mirrors solutions-cuda/softmax.cu."""
import os
import sys
import torch
import triton
...
def _kernel(x_ptr, y_ptr, D, BLOCK: tl.constexpr):
    ...
    m = tl.max(x); e = tl.exp(x - m); y = e / tl.sum(e)
...
def main(do_check):
    ...
    if do_check:
        xv = x.view(R, D)
        return tb.check("softmax", y.view(R, D), torch.softmax(xv, dim=1))
    return 0
