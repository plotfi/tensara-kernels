"""Triton solution for `max-normalize` — mirrors solutions-cuda/max-normalize.cu."""
import os
import sys
import torch
import triton
...
def main(do_check):
    ...
    if do_check:
        xv = x.view(R, D)
        return tb.check("max-normalize", y.view(R, D), xv / xv.abs().amax(1, keepdim=True))
    return 0


if __name__ == "__main__":
    tb.run("max-normalize", main)
