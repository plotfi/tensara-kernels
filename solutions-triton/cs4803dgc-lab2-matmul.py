"""Triton solution for `cs4803dgc-lab2-matmul` (C = A*B, square 512x512).
...
size 512 matches the CUDA harness so `run-bench.sh -C` compares like-for-like."""
...
def main(do_check):
    ...
    tb.preview(c, "C")
    if do_check:
        return tb.check("cs4803dgc-lab2-matmul", c.view(n, n), a.view(n, n) @ b.view(n, n), rtol=1e-2, atol=1e-2)
    return 0


if __name__ == "__main__":
    tb.run("cs4803dgc-lab2-matmul", main)
