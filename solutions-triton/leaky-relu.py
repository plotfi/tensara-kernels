"""Triton solution for `leaky-relu` — elementwise over n*m (mirrors solutions-cuda/leaky-relu.cu)."""
import os
import sys
import torch
import triton
...
def solution(x, y, total, alpha=0.01):
    BLOCK = 1024
...
def main(do_check):
    ...
    if do_check:
        ALPHA = 0.01
        return tb.check("leaky-relu", y, torch.nn.functional.leaky_relu(x, 0.01))
    return 0


if __name__ == "__main__":
    tb.run("leaky-relu", main)
