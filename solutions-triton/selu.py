"""Triton solution for `selu` — elementwise over n*m (mirrors solutions-cuda/selu.cu)."""
import os
import sys
import torch
import triton
...
def _kernel(x_ptr, y_ptr, n, BLOCK: tl.constexpr):
    ...
    y = 1.0507 * (tl.maximum(x, 0.0) + tl.minimum(0.0, 1.67326 * (tl.exp(x) - 1.0)))
...
def main(do_check):
    ...
    if do_check:
        ALPHA = 0.0
        return tb.check("selu", y, torch.nn.functional.selu(x))
    return 0
