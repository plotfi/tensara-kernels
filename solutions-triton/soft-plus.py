"""Triton solution for `soft-plus` — elementwise over n*m (mirrors solutions-cuda/soft-plus.cu)."""
import os
import sys
import torch
import triton
...
def main(do_check):
    ...
    if do_check:
        ALPHA = 0.0
        return tb.check("soft-plus", y, torch.nn.functional.softplus(x))
    return 0


if __name__ == "__main__":
    tb.run("soft-plus", main)
