// Correctness test for l1-norm: y = x / sum(|x|), per row.
...
int main() {
    ...
    test::check_cuda("l1-norm");
    d_y.download(h_out);

    for (size_t r = 0; r < B; r++) {
        double acc = 0.0;
        for (size_t j = 0; j < D; j++) acc += fabs(static_cast<double>(h_x[r * D + j]));
        float denom = static_cast<float>(acc);
        for (size_t j = 0; j < D; j++) h_ref[r * D + j] = h_x[r * D + j] / denom;
    }

    int bad = test::compare("l1-norm", h_out, h_ref, B * D, 1e-2f, 1e-3f);
    int rc = test::report("l1-norm", bad, B * D);
